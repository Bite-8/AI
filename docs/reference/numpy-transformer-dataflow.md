...
```bash
.venv/bin/python -m prototypes.numpy_transformer --prompt "Mark1のtransformer確認" --max-new-tokens 8 --temperature 0 --seed 42
```
...
| ファイル | 役割 |
|---|---|
| `prototypes/numpy_transformer/main.py` | CLI の入口。プロンプトを受け取り、トークナイズ、モデル実行、生成、ログ保存を行う。 |
| `prototypes/numpy_transformer/tokenizer.py` | 文字単位 tokenizer。文字列を token id に変換し、token id を文字列へ戻す。 |
| `prototypes/numpy_transformer/model.py` | NumPy Transformer 本体。Embedding、Self-Attention、FFN、LayerNorm、LM Head を実行する。 |
| `prototypes/numpy_transformer/decode.py` | 生成処理。最後の logits から次トークンを選び、逐次的に文章を伸ばす。 |
| `prototypes/numpy_transformer/train_min.py` | 最小学習デモ。Transformer 全体ではなく LM head の bias のみを更新する。 |
...
## 1. `prototypes/numpy_transformer/main.py`: 実行全体の制御

`prototypes/numpy_transformer/main.py` は Mark1 の入口である。大きくは以下の処理を行う。
...
## 2. `prototypes/numpy_transformer/tokenizer.py`: 文字列を token id に変換する
...
## 3. `prototypes/numpy_transformer/model.py`: Transformer 本体
...
## 4. `prototypes/numpy_transformer/decode.py`: 次 token を選んで文章を伸ばす
...
`prototypes/numpy_transformer/main.py` は実行結果を JSONL に追記する。
...
## 6. 学習デモ `prototypes/numpy_transformer/train_min.py`

`prototypes/numpy_transformer/train_min.py` は本格的な Transformer 学習ではない。教育用に、LM head の bias だけを SGD で更新する。
